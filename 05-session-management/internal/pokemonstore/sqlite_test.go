...
func TestSearchTeamSortByCaughtDate(t *testing.T) {
	...
	if _, err := store.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, 4, "Charmander", 10, "FIRE", 28, 28, "2024-01-10", 5); err != nil {
	...
	if len(latest) < 3 || latest[0].Name != "Squirtle" {
		t.Fatalf("expected Squirtle most recent catch, got %#v", latest)
	}
}

func TestUpdateHPByName(t *testing.T) {
	t.Parallel()
	...
	if err := store.UpdateHPByName("Pikachu", 0); err != nil {
		t.Fatalf("UpdateHPByName: %v", err)
	}

	pokemon, err := store.GetByName("Pikachu")
	if err != nil {
