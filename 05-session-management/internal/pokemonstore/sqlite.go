...
func (s *SQLiteStore) GetByName(name string) (domain.TeamPokemon, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return domain.TeamPokemon{}, fmt.Errorf("pokemon name is required")
	}
	row := s.db.QueryRow(`
		SELECT `+teamSelectColumns+`
		FROM team_pokemon
	...
	pokemon, err := scanTeamPokemon(row)
	if err == sql.ErrNoRows {
		return domain.TeamPokemon{}, fmt.Errorf("pokemon %q not found on team", name)
	}
	return pokemon, err
}

func (s *SQLiteStore) UpdateHPByName(name string, hp int) error {
	pokemon, err := s.GetByName(name)
	if err != nil {
		return err
	}
	if hp < 0 {
	...
	_, err = s.db.Exec(`UPDATE team_pokemon SET hp = ? WHERE id = ?`, hp, pokemon.ID)
	return err
}
...
func (s *SQLiteStore) init() error {
	...
	seeds := []struct {
		...
		{25, "Pikachu", 14, "ELECTRIC", 35, 35, "2024-03-05", "", 3},
		{7, "Squirtle", 15, "WATER", 40, 40, "2024-03-06", "", 4},
	}
