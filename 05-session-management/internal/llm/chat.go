...
// Complete returns a single non-streaming chat response.
...
const describeImageSystemPrompt = `You describe what a Pokémon trainer is showing in Kanto Season 1 terms.
Write one or two concise sentences. Mention location cues (forest, route, city, gym) when visible.`

// DescribeImage summarizes an image for session decision-making.
func (c *Client) DescribeImage(ctx context.Context, imageData []byte, mime, sessionContext string) (string, error) {
	if len(imageData) == 0 {
	...
	prompt := "Describe what the trainer is showing."
	if sessionContext = strings.TrimSpace(sessionContext); sessionContext != "" {
		prompt = fmt.Sprintf(
			"Describe what the trainer is showing. Note if this suggests a location or context change relative to: %s",
			sessionContext,
		)
	}
	messages := []llms.MessageContent{
		{
			Role:  llms.ChatMessageTypeSystem,
			Parts: []llms.ContentPart{llms.TextPart(describeImageSystemPrompt)},
		},
		{
			Role: llms.ChatMessageTypeHuman,
			Parts: []llms.ContentPart{
				llms.BinaryPart(mime, imageData),
				llms.TextPart(prompt),
			},
		},
	}
	resp, err := c.model.GenerateContent(ctx, messages, llms.WithTemperature(c.settings.Temperature))
	if err != nil {
		return "", err
	}
	if len(resp.Choices) == 0 {
