...
func TestPlanToolCallNoRedirectOnHealthSessionMemory(t *testing.T) {
	t.Parallel()

	plan := planToolCall("Bulbasaur health updated to 0", &llms.FunctionCall{
		Name:      "session_memory",
		Arguments: `{"query":"Bulbasaur health updated to 0"}`,
	})
	if plan.corrected {
		t.Fatalf("expected no route correction for health update, got %#v", plan)
	}
	if plan.name != "session_memory" {
		t.Fatalf("expected session_memory, got %q", plan.name)
	}
}
