...
type AppScreen = 'feed' | 'team' | 'sessions';
...
function App() {
    ...
    async function refreshTeam() {
        try {
            const teamResult = await ListMyTeam();
            setTeam(teamResult ?? []);
            setTeamError('');
        } catch {
    ...
    useEffect(() => {
        ...
        async function loadInitialData() {
            ...
            setTeamError('');
            try {
                const [teamResult, situationResult, sessionsResult] = await Promise.all([
                    ListMyTeam(),
                    GetCurrentSituation(),
                    ListSessions(),
                ]);
                if (!cancelled) {
                    ...
                    const active =
                        (sessionsResult ?? []).find((session) => session.active) ?? null;
                    activeSessionIdRef.current = active?.id ?? null;
                    setActiveSession(active);
                }
            } catch {
    ...
    useEffect(() => {
        ...
        const unsubscribeTraceReset = EventsOn('agent:trace-reset', () => {
            setTraceSteps([]);
            setAnalysis('');
        });
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            await Promise.all([refreshSituation(), refreshSessions(), refreshTeam()]);
            setPrompt('');
            clearImage();
        } catch (err) {
            const message =
